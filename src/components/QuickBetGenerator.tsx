    ...
    <Card className="card-glass border-primary/30 shadow-[0_0_30px_-5px_hsl(var(--primary)/0.15)] relative overflow-hidden">
      ...
      <CardContent className="space-y-5 relative">
        ...
        <Button
          ...
          disabled={isSpinning || !selected || selected.id === "loteca" || (isFederal && (loadingFedHistory || !!fedHistoryError))}>
          {isSpinning ?
          <RefreshCw className="w-5 h-5 mr-2 animate-spin" /> :
          (isFederal ? <Ticket className="w-5 h-5 mr-2" /> : <Dices className="w-5 h-5 mr-2" />)
          }
          {freeGenerations > 0
            ? `${isFederal ? "Gerar Bilhetes" : "Gerar Aposta"} (${freeGenerations} restante${freeGenerations > 1 ? "s" : ""})`
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        setCopiedFedIdx(idx);
                        setTimeout(() => setCopiedFedIdx(null), 1500);
